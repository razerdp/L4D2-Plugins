...
RegisterCvars()
{
	...
	/* 关于升级 */
	LvUpSP						= CreateConVar("rpg_LvUp_SP",		"15",	"升级获得的属性点", CVAR_FLAGS, true, 0.0);
	...
	LvUpExpRate				= CreateConVar("rpg_LvUp_Exp_Rate",	"16000",	"升级Exp系数: 升级经验=升级系Exp数*(当前等级+1)", CVAR_FLAGS, true, 1.0);
	...
	cv_vippropsE					= CreateConVar("rpg_vipropsE",				"18",	"至尊VIP5的每回合免费补给数量", CVAR_FLAGS);
	cv_vippropsF					= CreateConVar("rpg_vipropsF",				"20",	"至尊VIP6的每回合免费补给数量", CVAR_FLAGS);


	/* 紫炎坦克 */
	sm_supertank_bossratio	  = CreateConVar("sm_supertank_bossratio",   "7.5", "紫炎坦克出现几率(默认是5/100)(另一半机率为4阶段Tank)",    CVAR_FLAGS);
	sm_supertank_Killerratio	  = CreateConVar("sm_supertank_Killerratio",   "2.0", "闪电坦克出现几率(默认是5/100)(默认关闭,已经无效)",    CVAR_FLAGS);
	
	sm_supertank_bossrange	  = CreateConVar("sm_supertank_bossrange",   "40.0", "紫炎坦克屏幕抖动影响范围",    CVAR_FLAGS);
	//sm_supertank_Killerrange	  = CreateConVar("sm_supertank_Killerrange",   "40.0", "闪电坦克屏幕抖动影响范围",    CVAR_FLAGS);

	/* 超级坦克生命值 */
	sm_supertank_health_max	  = CreateConVar("sm_supertank_health_max",   "30000", "超级坦克第一阶段生命值(默认血量)",    CVAR_FLAGS);
	...
	sm_supertank_health_Killer		= CreateConVar("sm_supertank_health_Killer", "18000",  "闪电坦克生命值(默认血量)",  CVAR_FLAGS);
	...
	*/
	...
	rpg_gamedifficulty_tankHP_lv1	 = CreateConVar("rpg_gamedifficulty_tankHP_lv1",   "60000, 40000, 36000", "难度自动平衡等级1: (Tank血量分别对应: 4阶段, 紫炎, 闪电)",    CVAR_FLAGS);
	rpg_gamedifficulty_tankHP_lv2	 = CreateConVar("rpg_gamedifficulty_tankHP_lv2",   "90000, 80000, 72000", "难度自动平衡等级2: (Tank血量分别对应: 4阶段, 紫炎, 闪电)",    CVAR_FLAGS);
	rpg_gamedifficulty_tankHP_lv3	 = CreateConVar("rpg_gamedifficulty_tankHP_lv3",   "180000, 160000, 148000", "难度自动平衡等级3: (Tank血量分别对应: 4阶段, 紫炎, 闪电)",    CVAR_FLAGS);
	rpg_gamedifficulty_tankHP_lv4	 = CreateConVar("rpg_gamedifficulty_tankHP_lv4",   "360000, 320000, 256000", "难度自动平衡等级4: (Tank血量分别对应: 4阶段, 紫炎, 闪电)",    CVAR_FLAGS);
	rpg_gamedifficulty_tankHP_lv5	 = CreateConVar("rpg_gamedifficulty_tankHP_lv5",   "720000, 640000, 312000", "难度自动平衡等级5: (Tank血量分别对应: 4阶段, 紫炎, 闪电)",    CVAR_FLAGS);
	rpg_gamedifficulty_tankHP_lv6	 = CreateConVar("rpg_gamedifficulty_tankHP_lv6",   "1440000, 1280000, 624000", "难度自动平衡等级6: (Tank血量分别对应: 4阶段, 紫炎, 闪电)",    CVAR_FLAGS);
	rpg_gamedifficulty_tankHP_lv7	 = CreateConVar("rpg_gamedifficulty_tankHP_lv7",   "2880000, 2560000, 1228000", "难度自动平衡等级7: (Tank血量分别对应: 4阶段, 紫炎, 闪电)",    CVAR_FLAGS);
...
public Action:Command_RPGGM(Client, args)
{
	if (IsValidPlayer(Client, false))
	{
		...
		Intelligence[Client] = 10000;
		SkillPoint[Client] = 500;
		Crits[Client] = 10000;
		CritMin[Client] = 10000;
		CritMax[Client] = 10000;
	}
}
...
public Action:Command_Say(Client, args)
{
	...
	if (GetClientTeam(Client) == 2)
	{
		if (mode == 0)
		{
			...
			if (VIP[Client] == 6)
			{
				CPrintToChatAll("[服主VIP]%N: {blue}%s", Client, sText);
			}
		}
		if (mode == 1)
		{
...
public Action:PlayerLevelAndMPUp(Handle:timer, any:target)
{
	if(IsClientInGame(target))
	{
		...
		if(EXP[target] >= GetConVarInt(LvUpExpRate)*(Lv[target]+1))
		{
			...
			if (Lv[target] == 10 || Lv[target] == 20 || Lv[target] == 30 || Lv[target] == 40 || Lv[target] == 50 || Lv[target] == 60 || Lv[target] == 70 || Lv[target] == 80 || Lv[target] == 90 || Lv[target] == 100 || Lv[target] == 110 || Lv[target] == 120 || Lv[target] == 130 || Lv[target] == 140 || Lv[target] == 150 || Lv[target] == 160 || Lv[target] == 170 || Lv[target] == 180 || Lv[target] == 190 || Lv[target] == 200 || Lv[target] == 210 || Lv[target] == 220 || Lv[target] == 230 || Lv[target] == 240 || Lv[target] == 250 || Lv[target] == 260 || Lv[target] == 270 || Lv[target] == 280 || Lv[target] == 290 || Lv[target] == 300 || Lv[target] == 310 || Lv[target] == 320 || Lv[target] == 330 || Lv[target] == 340 || Lv[target] == 350 || Lv[target] == 360 || Lv[target] == 370 || Lv[target] == 380 || Lv[target] == 390 || Lv[target] == 400 || Lv[target] == 410 || Lv[target] == 420 || Lv[target] == 430 || Lv[target] == 440 || Lv[target] == 450 || Lv[target] == 460 || Lv[target] == 470 || Lv[target] == 480 || Lv[target] == 490 || Lv[target] == 500 || Lv[target] == 510 || Lv[target] == 520 || Lv[target] == 530 || Lv[target] == 540 || Lv[target] == 550 || Lv[target] == 560 || Lv[target] == 570 || Lv[target] == 580 || Lv[target] == 590 || Lv[target] == 600 || Lv[target] == 610 || Lv[target] == 620 || Lv[target] == 630 || Lv[target] == 640 || Lv[target] == 650 || Lv[target] == 660 || Lv[target] == 670 || Lv[target] == 680 || Lv[target] == 690 || Lv[target] == 700 || Lv[target] == 710 || Lv[target] == 720 || Lv[target] == 730 || Lv[target] == 740 || Lv[target] == 750 || Lv[target] == 760 || Lv[target] == 770 || Lv[target] == 780 || Lv[target] == 790 || Lv[target] == 800 || Lv[target] == 810 || Lv[target] == 820 || Lv[target] == 830 || Lv[target] == 840 || Lv[target] == 850 || Lv[target] == 860 || Lv[target] == 870 || Lv[target] == 880 || Lv[target] == 890 || Lv[target] == 900 || Lv[target] == 910 || Lv[target] == 920 || Lv[target] == 930 || Lv[target] == 940 || Lv[target] == 950 || Lv[target] == 960 || Lv[target] == 970 || Lv[target] == 980 || Lv[target] == 990 || Lv[target] == 1000 || Lv[target] == 1010 || Lv[target] == 1020 || Lv[target] == 1030 || Lv[target] == 1040 || Lv[target] == 1050 || Lv[target] == 1060 || Lv[target] == 1070 || Lv[target] == 1080 || Lv[target] == 1090 || Lv[target] == 1100 || Lv[target] == 1120 || Lv[target] == 1130 || Lv[target] == 1140 || Lv[target] == 1150 || Lv[target] == 1160 || Lv[target] == 1170 || Lv[target] == 1180 || Lv[target] == 1190 || Lv[target] == 1220 || Lv[target] == 1230 || Lv[target] == 1240 || Lv[target] == 1250 || Lv[target] == 1260 || Lv[target] == 1270 || Lv[target] == 1280 || Lv[target] == 1290 || Lv[target] == 1300)
			{
...
public DAKAILI(Client)
{
	if(Libao[Client] > 0)
	{
		...
		switch (jinwuqi)
		{
			...
			{               
				XB[Client] += 500;               
				CPrintToChatAll("\x05【升级礼包】玩家%N打开获得求生币500个!", Client);           
			}
			case 3:
			{
				Lv[Client] += 1;
...
public PlayerSignXHToday(Client)
{
	if (IsValidPlayer(Client) && IsPasswordConfirm[Client])
	{
		...
		if (today > 0 && EveryDaySign[Client] > -1 && EveryDaySign[Client] != today && everyday1[Client] <= 15)
		{
			...
			CPrintToChatAll("{olive}[每日签到]\x05玩家 \x03 %N \x05已经在今日签到了,获得奖励\x03%dEXP,%d$\x05和\x03随机卷轴\x05!积累签到:%d次 求生币:\x03 500 \x05个", Client, SIGNAWARD_EXP[Client], SIGNAWARD_CASH[Client], everyday1[Client], XB[Client]);
		}
